        ...
        void UpdateAI(const uint32 diff) {
            ...
            if (IsHeroic()) {
                if (me->GetHealthPct() < 25 && phase != PHASE_FINAL) {
                    me->SummonCreature(NPC_SPIKED_TENTACLE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f, TEMPSUMMON_CORPSE_DESPAWN);
                }
                ...
                /*if (me->GetHealthPct() < 1 && phase != PHASE_FINAL) {
                    if (GameObject * pGoFloor = instance->GetGameObject(instance->GetData64(GO_CHOGALL_FLOOR)))
